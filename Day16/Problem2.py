...
class Terrain:
    ...
    # Resets the terrain to it's initial state
    def reset(self):
        self.met_tiles.clear()
...
# Dictionnary containig the possible directions for each corner
corners = {(0,0):[terrain.directions.right, terrain.directions.down], 
           (0, height-1):[terrain.directions.right, terrain.directions.up], 
           (width-1, 0):[terrain.directions.left, terrain.directions.down], 
           (width-1,height-1):[terrain.directions.left, terrain.directions.up]}

# Configuration with the largest number of tiles energized
# (initial_ray_pos, initial_ray_dir, nbr_tiles_energized)
max_energized = (0, 0, 0) 

# Try all the possible starting positions and directions for the light ray
for y in range(height):
    # Determine all the possible x positions for this y postion
    if y == 0 or y == height-1:
        possible_start_x_pos = range(width)
    else:
        possible_start_x_pos = [0, width-1]
    
    # For all those possible x positions
    for x in possible_start_x_pos:
        # Determine all the possible directions for the starting position (x,y)
        if (x,y) in corners:
            possible_dir = corners[(x,y)]
        else:
            if x == 0:
                possible_dir = [terrain.directions.right]
            elif x == width-1:
                possible_dir = [terrain.directions.left]
            else:
                if y == 0:
                    possible_dir = [terrain.directions.down]
                elif y == height-1:
                    possible_dir = [terrain.directions.up]
                else:
                    print("The coordinate", (x,y), "is not an edge")
                    exit()

        # The linear coordinate corresponding to the position (x,y)
        pos = y * width + x

        # For all the possible directions
        for dir in possible_dir:
            # Reinitialize the terrain
            terrain.reset()

            # Create the light ray with the right initial position and direction
            ray = LightRay(pos, dir)
            ...
            # Check if the number of energize tiles is bigger than the previous max and actualize it accordingly
            energized_tiles = terrain.energized_tiles()
            if energized_tiles > max_energized[-1]:
                max_energized = (pos, dir, energized_tiles)

print("The maximum amount of tiles energized is", max_energized[-1], ", possible with a ray starting at position", (max_energized[0]//width, max_energized[0]%width), ", in the direction", terrain.directions.get_char(max_energized[1]))
